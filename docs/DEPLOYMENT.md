...
- Security headers on every response, including a
  ...
  Archivo Black, DM Sans and IBM Plex Mono from Google Fonts — **removing
  those two origins from the CSP will break the site's typography.**
  `script-src`/`style-src` also allow `'unsafe-inline'` because this
  variant's entire CSS and JS live inline in `index.html` with no build
  step to hash or externalize them.

**If you ever add a new third-party resource** (analytics, a map embed, a
different font host, an ordering widget script), you must widen the
relevant CSP directive in `netlify.toml` first, or it will silently fail to
load.
...
- [ ] Google Fonts still load — check that `Archivo Black`/`DM Sans`/`IBM
      Plex Mono` render, not a system-font fallback.
- [ ] Response headers include the security headers from `netlify.toml`
      (check with browser dev tools or `curl -I`).
- [ ] Every "Call" and "Order" link for all three locations works.
